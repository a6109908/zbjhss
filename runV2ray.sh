...
if [ ! -z "${CFToken}" ]
then
  sed -i "s|\"cf_token\": \"679asdf\"|\"cf_token\": \"${CFToken}\"|g" "/etc/v2ray/config.json"

fi
if [ ! -z "${CFAccoundID}" ]
then
  sed -i "s|\"cf_accound_id\": \"asdf1234\"|\"cf_accound_id\": \"${CFAccoundID}\"|g" "/etc/v2ray/config.json"

fi
if [ ! -z "${CFZoneID}" ]
then
  sed -i "s|\"cf_zone_id\": \"owieur123\"|\"cf_zone_id\": \"${CFZoneID}\"|g" "/etc/v2ray/config.json"
